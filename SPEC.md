...
- enforce file count/size, fact, edge, evidence, traversal-depth, output-size, and wall-time limits; the library edge/evidence budgets, configured ignore-glob policy, and private CLI output-size enforcement are bounded, while library output/time enforcement remains open;
