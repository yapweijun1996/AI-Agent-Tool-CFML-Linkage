...
The foundation is easy to run and audit, with no supply-chain dependency added for path safety. Node's platform path semantics still require cross-platform tests before release. The package is not yet a public API: there is no CLI entry point or export map, and `private` prevents accidental publication.
