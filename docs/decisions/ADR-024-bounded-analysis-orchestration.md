...
`src/index.js` and the package `exports` field provide the private library boundary. The CLI `analyze` and `index` commands use the same bounded pipeline and preserve the existing JSON/stdout and diagnostic/stderr envelope. Query commands remain unimplemented until graph persistence/query input wiring is defined.
...
- This does not establish full CFML grammar coverage, runtime inference, all configuration limits, public-release compatibility, or application execution.
