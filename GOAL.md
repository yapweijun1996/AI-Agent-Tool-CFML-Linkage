...
The project must track **Planned → Implemented → Verified → Released** independently. M0/M1, bounded M2 parser/scanner/Fact extraction, bounded M3–M6 linkage, bounded M7 queries/orchestration, bounded M8 robustness/adversarial safe-failure evidence, and bounded M9 checks/package smoke/Node host compatibility/release-parity audit and the T-047 CI workflow and T-048 library evidence-budget enforcement are implemented; full parser coverage, graph persistence, public package, and release remain incomplete/unreleased; bounded query-command integration is verified.
