...
- file/byte/fact/edge/evidence/traversal/output/time limits and query result/depth/visited-node limits; configured ignore-glob matching, the library `max_edges`/`max_evidence`, and private CLI serialized `max_output_bytes` caps are verified, while library output/time caps remain open;
