...
- file/byte/fact/edge/evidence/traversal/output/time limits and query result/depth/visited-node limits; configured ignore-glob/hidden-file policy matching, the library `max_edges`/`max_evidence` and `serializeAnalysis` output budget, and private CLI serialized `max_output_bytes` caps are verified; T-038–T-039 define the remaining private-library wall-time and cross-budget checks, which are not yet verified;
...
**T-001–T-006, T-010–T-014, T-020–T-037, T-040–T-046, and T-048–T-051 passed for their bounded scopes:** contract schemas/examples, identity/order, confidence/completeness, configuration, fixture-manifest, root-guard containment, deterministic discovery, configured ignore-glob/hidden-file policy matching, snapshot fingerprint, symlink, limit, strict decoding, source maps, CLI envelope, stderr separation, cache invalidation/corruption, parser unavailable/partial/failure handling, bounded CFML scanning, structural Fact extraction, golden expectations, immutable index lookups, literal path resolution, bounded Graph IR construction/validation, reverse adjacency, bounded CFC mapping/inheritance/instantiation/invoke/method resolution, ordered scope production/consumption/override and resource-limit handling, web-flow condition/wrapper resolution, dynamic/generated/SQL-dynamic preservation, bounded `cfquery`/`queryExecute` SQL and structural repository/action resolution, immutable GraphSnapshot queries, exact-selector ambiguity, deterministic explanations, traversal bounds, composed analysis orchestration, parser-time drift, repeatability, content drift, cycle safety, resource limits, inert adversarial safe-failure fixtures, unsupported regions, no-execution checks, and final Graph edge-budget/hidden-file-policy enforcement pass; `npm test` reports 79 passed and 0 failed, and the produced Fact/Graph/analysis IR validates against schema. T-037 private-library serialization is verified in ADR-029; T-038–T-039 remain defined but unimplemented and unverified. This uses no project dependency beyond Node built-ins. Runtime producer repeatability beyond the current foundation tests, graph persistence, broader Graph linkage behavior, public package publication, Lucee/Adobe/browser, hosted CI, and broader engine checks remain unverified; local private package self-import/CLI smoke, Node host evidence, and bounded release/security/parity audit passed.
