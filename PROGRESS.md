...
| CLI/library/API | Yes | Partial | Partial | No | `src/analyzer.js`, `src/index.js`, package `exports`, `src/cli.js`, `test/analyzer.test.js`, and `test/cli.test.js`; bounded private `analyzeProject`/`analyze`/`index` composition passes, while query commands, public release, and full grammar remain open |
...
6. **M7 partial:** T-035 provides bounded immutable graph queries and deterministic evidence explanations, and T-036 composes the bounded stages behind a private library/CLI analysis entry; query-command wiring, broad output integration, and query contract release remain open.
