...
> **Status: DOCUMENTATION SYNC VERIFIED / M1 FOUNDATION VERIFIED / M2–M6 BOUNDED IMPLEMENTATION PARTIAL.** This report is evidence-based and intentionally separates plan from product state.
...
| Verification | Contract/fixture checks, produced Fact/Graph IR schema validation, and `npm test` foundation/parser-adapter/scanner/Fact/index/resolution/Graph/CFC/scope/web-flow/dynamic-evidence tests passed (60/60); broader implementation checks remain unavailable |
...
- **Lifecycle:** prototype / M1 foundation verified, M2 bounded parser/Fact extraction partial, M3 bounded literal resolution/Graph IR, M4 bounded CFC, M5 bounded scope/web-flow, and M6 bounded dynamic-evidence preservation partial; broader resolver not started.
- **Current product boundary:** one planned static linkage analyzer with implemented internal root-guard, snapshot, decoder, private CLI, cache, bounded parser/scanners, Fact extractor, indexes, literal resolver, Graph builder/validator, reverse adjacency, CFC resolver, scope resolver, web-flow resolver, and dynamic/generated/SQL-dynamic evidence handling; no public package boundary exists.
...
| Root guard | Yes | Yes | Yes | No | `src/root-guard.js`, `test/root-guard.test.js`, 6 focused cases; suite 60/60 |
...
- Implemented and verified the Node root guard, deterministic snapshot/discovery, strict decoder/source map, private CLI envelope, disposable cache, fail-closed parser adapter, bounded CFML/web scanners, fixture-backed Fact extractor, immutable indexes, bounded literal/CFC/scope/web-flow resolvers, dynamic/generated/SQL-dynamic evidence handling, and bounded Graph IR builder/validator/reverse adjacency for T-010–T-014/T-020–T-033; no full parser grammar or broad linkage resolution exists.
...
5. **M6 partial:** T-033 preserves dynamic/generated/SQL-dynamic evidence; SQL semantics, runtime mappings, and target inference remain open.
6. **T-031/T-032/T-033 boundary:** Scope uses bounded literal include order and lexical references; web-flow uses supplied literal targets and explicit conditions; dynamic evidence preserves bounded expressions only. Do not claim runtime behavior.
7. **Public package/runtime contract:** Node `>=20`, a private package manifest, and a private CLI entry are established for the foundation; public exports, full CLI commands, CI matrix, and release metadata remain unknown.
8. **Compatibility evidence:** Lucee/Adobe/browser/runtime claims cannot be made until environments and fixtures exist.
...
| Rule/document/config discovery | Passed | M1–M6 bounded implementation and contract files were inspected |
...
| Build/type/lint/unit/integration/E2E | Partial: `npm test` passed 60 foundation/parser-adapter/scanner/Fact/index/resolution/Graph/CFC/scope/web-flow/dynamic-evidence tests | No broader Graph linkage build/type/lint/integration/E2E suite exists |
...
**Next task: M6 / T-034 — add bounded SQL table/datasource and structural repository/action resolution.** Preserve unresolved records and do not claim full parser coverage.
