...
> **Status: CLI CONFIGURATION + LIBRARY EVIDENCE BUDGET VERIFIED / M1 FOUNDATION VERIFIED / M2–M9 BOUNDED IMPLEMENTATION PARTIAL.** This report is evidence-based and intentionally separates plan from product state.
...
| Root/configuration/limits contract | Yes | Yes | Yes | No | ADR-004, config schema/example, `src/cli.js`, `test/cli.test.js`, and policy invariant check; the private CLI rejects schema-shape and value violations before root admission, while the library evidence budget is enforced and other library-wide limits remain open |
...
| Root guard | Yes | Yes | Yes | No | `src/root-guard.js`, `test/root-guard.test.js`, 6 focused cases; suite 75/75 |
...
| Determinism/cache/cycle/limit robustness | Yes | Partial | Partial | No | `test/snapshot.test.js`, `test/cache.test.js`, `test/graph-query.test.js`, `test/analyzer.test.js`, `test/cli.test.js`, and resolver/scanner limit cases; repeatability, content drift, cache invalidation/corruption, parser-time drift, cycle safety, bounded CLI output, and bounded-result evidence pass; the configured library evidence budget is enforced, while library output and full time budgets remain open |
...
| Build/type/lint/unit/integration/E2E | Partial: `npm test` passed 75 foundation/parser-adapter/scanner/Fact/index/resolution/Graph/CFC/scope/web-flow/dynamic-evidence/SQL-repository/query/orchestration/robustness/adversarial-fixture/CLI-query/configuration/evidence-budget tests; T-047 workflow is structurally reviewed but not hosted-run | No broader Graph linkage build/type/lint/integration/E2E suite exists |
