...
| Verification | Root-guard/snapshot/decoder/CLI tests pass; no release candidate exists |
...
The current repository has no release version. `0.1` is a design/documentation version only; the private `package.json`, CLI envelope, and root-guard/snapshot/decoder foundation are not a released package or linkage runtime.
